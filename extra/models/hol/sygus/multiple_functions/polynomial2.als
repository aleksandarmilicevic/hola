...
--------------------------------------------------------------------------------
-- Specification
-- (https://github.com/rishabhs/sygus-comp14/blob/master/benchmarks/multiple-functions/polynomial2.sl)
--------------------------------------------------------------------------------
...
pred synth[addExpr1, addExpr2: Node] {
  all envI: IntVar -> one Int {
    some eval: IntNode->Int + BoolNode->Bit when {
      ...
      plus[eval[addExpr1], eval[addExpr2]] = minus[eval[X], eval[Y]]
    }
  }
}
